...
#  Fecha: 03/06/2025
#  Descripción: Script para comparar respuestas generadas por Gemini con las respuestas correctas de referencia. 
...
file_correctas = "preguntas_gemini.xlsx"
...
for index, row in df_respuestas.iterrows():
    ...
    if pregunta.startswith("Q") and pregunta[1:].isdigit() and 1 <= int(pregunta[1:]) <= 10:
        ...
        if not fila_correcta.empty:
            columna_correcta = f"Correct_Option_{pregunta[1:]}"
            ...
            # Comparar solo la primera letra
            if respuesta.strip()[0:1].upper() == respuesta_correcta.strip()[0:1].upper():
                resultado = "Correcta"
            else:
...
# Guardar en Excel
output_file = "resultados_gemini.xlsx"
